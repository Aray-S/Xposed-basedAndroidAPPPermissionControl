...
public class GetLocation extends BaseHook {
    ...
    public static boolean hook(final Context context) {
        ...
        if (!hookBaseBundleGet(context)) status = false;
        if (!hookLocationCreateFromParcel(context)) status = false;
        if (!hookLocationManagerAddNmeaListener1(context)) status = false;
        if (!hookLocationManagerAddNmeaListener2(context)) status = false;
        if (!hookGeofenceSetCircularRegion(context)) status = false;
        if (!hookPlaceLikelihoodBufferGetCount(context)) status = false;
        if (!hookPlaceLikelihoodBufferGet(context)) status = false;
        return status;
    }
    ...
    private static boolean hookBaseBundleGet(final Context context) {
        ...
        try {
            ...
            Member member = getClassForName(android.os.BaseBundle.class.getName()).getDeclaredMethod("get", String.class);
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    Object object = param.getResult();
                    if (param.getThrowable() != null || object == null) return;
                    if (!param.args[0].equals("location")) return;
    ...
    private static boolean hookLocationManagerAddNmeaListener1(final Context context) {
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable { param.setResult(false); }
            });
        } catch (Exception e) {
            XposedBridge.log(e);
            return false;
    ...
    private static boolean hookPlaceLikelihoodBufferGetCount(final Context context) {
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable { param.setResult(0); }
            });
        } catch (ClassNotFoundException e) {
            return false;
        } catch (Exception e) {
    ...
    private static boolean hookPlaceLikelihoodBufferGet(final Context context) {
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable { param.setResult(null); }
            });
