...
public class GetApplications extends BaseHook {
    ...
    public static boolean hook(final Context context, final int uid) {
        ...
        boolean status = true;
        if (!hookGetRunningAppProcesses(context, uid)) { status = false; }
        if (!hookGetRunningServices(context, uid)) { status = false; }
        if (!hookGetInstalledProviders(context)) { status = false; }
        if (!hookGetInstalledProvidersForProfile(context)) { status = false; }
        if (!hookIntentCreateFromParcelOrPackage(context)) { status = false; }
        if (!hookGetInstalledApplications(context, uid)) { status = false; }
        if (!hookGetInstalledPackages(context, uid)) { status = false; }
        if (!hookGetPackagesHoldingPermissions(context, uid)) { status = false; }
        if (!hookQueryIntentActivities(context, uid)) { status = false; }
        if (!hookGetPreferredPackages(context, uid)) { status = false; }
        if (!hookQueryIntentActivityOptions(context, uid)) { status = false; }
        if (!hookQueryIntentContentProviders(context, uid)) { status = false; }
        if (!hookQueryIntentServices(context, uid)) { status = false; }
        return status;
    }
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (runningAppProcessInfoList == null) {return;}
                    for (int i = 0; i < runningAppProcessInfoList.size(); i++) {
                        if (runningAppProcessInfoList.get(i).uid == uid) { continue; }
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (runningServiceInfoList == null) {return;}
                    for (int i = 0; i < runningServiceInfoList.size(); i++) {
                        if (runningServiceInfoList.get(i).uid == uid) { continue; }
    ...
    private static boolean hookGetInstalledProviders(final Context context) {
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable { param.setResult(new ArrayList<>()); }
            });
        } catch (Exception e) {
            XposedBridge.log(e);
            return false;
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (applicationInfoList == null) {return;}
                    for (int i = 0; i < applicationInfoList.size(); i++) {
                        if (applicationInfoList.get(i).uid == uid) { continue; }
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    for (int i = 0; i < packages.size(); i++) {
                        if (packages.get(i).applicationInfo.uid == uid) { continue; }
                        packages.remove(i);
                        i--;
                    }
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    for (int i = 0; i < packageInfoList.size(); i++) {
                        if (packageInfoList.get(i).applicationInfo.uid == uid) { continue; }
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (resolveInfoList == null) { return; }
                    for (int i = 0; i < resolveInfoList.size(); i++) {
                        if (resolveInfoList.get(i).activityInfo.applicationInfo.uid == uid) { continue; }
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (resolveInfoList == null) { return; }
                    for (int i = 0; i < resolveInfoList.size(); i++) {
                        if (resolveInfoList.get(i).applicationInfo.uid == uid) { continue; }
                        resolveInfoList.remove(i);
                        i--;
                    }
                    param.setResult(resolveInfoList);
        ...
        try {
            ...
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    for (int i = 0; i < resolveInfoList.size(); i++) {
                        if (resolveInfoList.get(i).serviceInfo.applicationInfo.uid == uid) { continue; }
                        resolveInfoList.remove(i);
                        i--;
                    }
                    param.setResult(resolveInfoList);
