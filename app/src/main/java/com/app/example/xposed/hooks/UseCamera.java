...
public class UseCamera extends BaseHook {
    ...
    public static boolean hook(final Context context) {
        ...
        if (!hookCameraGetCameraInfo(context)) status = false;
        if (!hookCameraGetNumberOfCameras(context)) status = false;
        if (!hookCameraOpenCamera(context)) status = false;
        if (!hookCameraOpenLegacyCamera(context)) status = false;
        if (!hookCameraManager(context)) status = false;
        return status;
    }
    ...
    private static boolean hookCameraGetNumberOfCameras(final Context context) {
        ...
        try {
            if (Build.VERSION.SDK_INT < minSdk || Build.VERSION.SDK_INT > maxSdk) return false;
            Member member = getClassForName(android.hardware.Camera.class.getName()).getDeclaredMethod("getNumberOfCameras");
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                @Override
    ...
    private static boolean hookCameraOpenCamera(final Context context) {
        ...
        try {
            if (Build.VERSION.SDK_INT < minSdk || Build.VERSION.SDK_INT > maxSdk) return false;
            Member member = getClassForName(android.hardware.Camera.class.getName()).getDeclaredMethod("open", int.class);
            XposedBridge.hookMethod(member, new XC_MethodHook() {
                @Override
                protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                    param.setResult(new java.lang.RuntimeException("privacy"));
    ...
    private static boolean hookCameraManager(final Context context) {
        int minSdk = 21;
        int maxSdk = Integer.MAX_VALUE;
        try {
            if (Build.VERSION.SDK_INT < minSdk || Build.VERSION.SDK_INT > maxSdk) {return false;}
